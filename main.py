...
def tf_idf(summary_json, transcript_result, log_queue=None, reel=False): 
    ...
    try:
        # Fit vectorizer on transcript segments only once
        vectorizer = TfidfVectorizer(stop_words='english')
        tfidf_matrix = vectorizer.fit_transform(documents)

        log("Vectorizing subheadings...")
        subheading_vectors = vectorizer.transform(subheadings)

        # Compute cosine similarity matrix
        log("Computing cosine similarity...")
        similarity_matrix = cosine_similarity(subheading_vectors, tfidf_matrix)

        for i, subheading in enumerate(subheadings):
            log(f"Processing subheading: {subheading}")

            # Get top N matches per subheading
            top_indices = similarity_matrix[i].argsort()[::-1][:3]
            matches = []

            for idx in top_indices:
                segment = transcript_result["segments"][idx]
                matches.append({
                    "text": segment["text"],
                    "start": segment["start"],
                    "end": segment["end"],
...
def screenshots_matchs(content_id, search_results, video_path, screenshots_dir, index=0, log_queue=None):
    ...
    def process_match(result, idx):
        subheading = result['subheading']
        matches = result.get('matches')

        if not matches:
            log(f"No matches found for subheading: {subheading}")
            return None

        match = matches[0]
        start_time = seconds_to_ffmpeg_timestamp(match["start"])
        screenshot_id = f"{subheading.replace(' ', '_')}_{idx}"
        local_screenshot_path = os.path.join(screenshots_dir, f"{screenshot_id}.png")
        s3_path = f"screenshots/{content_id}/{screenshot_id}.png"
        s3_url = f"https://blooogerai.s3.amazonaws.com/{s3_path}"
    ...
    # Run all screenshot+upload operations in parallel threads
    with ThreadPoolExecutor(max_workers=5) as executor:
        futures = [
            executor.submit(process_match, result, index + i)
        ...
        for future in as_completed(futures):
            try:
                future.result()  # If needed, can log or handle the result
            except Exception as e:
                log(f"Error processing screenshot task: {e}")
...
async def process_youtube_url(url, product_data=None, related_posts=None, log_queue=None):
    ...
    try:
        ...
        yt = Youtube()
        log("Downloading audio...")
        audio_path, length = await run_in_threadpool(yt.download_audio_from_url, url)
        log(f"Downloaded audio: {audio_path}")

        log("Downloading video...")
        video_path, video_length = await run_in_threadpool(yt.download_video, url)
        ...
        content_id = os.path.basename(os.path.dirname(audio_path))
        base_path = f"./data/youtube/{content_id}"
        ...
        if os.path.exists(transcript_file):
            ...
            # Generate new transcript and create transcript_data as before
            transcript_result = await run_in_threadpool(transcribe, audio_path, log_queue)
            transcript_data = {
                "full_text": transcript_result["transcription"],
                "segments": transcript_result["segments"],
                "metadata": {
        ...
        # Process subheadings and perform TF-IDF analysis
        log("Performing TF-IDF analysis and search...")
        search_results = await run_in_threadpool(tf_idf, summary_json, transcript_result)
        # search_results = tf_idf(summary_json, transcript_result)
        try:
            log("Generating screenshots for search results...")
            video_path = f"./data/youtube/{content_id}/"
            ...
            # screenshots_matchs(content_id, search_results, video_path, screenshots_dir)
            await run_in_threadpool(screenshots_matchs, content_id,  search_results, video_path, screenshots_dir)
...
async def make_reel(prompt, reel_id, key=None, product_info=None, log_queue=None):
    ...
    async def process_video(video):
        try:
            content_id = await process_youtube_url(
                video['url'],
                product_data=product_info,
                related_posts=related,
                log_queue=log_queue
            ...
            if content_id:
                transcript_path = f"./data/youtube/{content_id}/transcript.txt"
                if os.path.exists(transcript_path):
                    with open(transcript_path, 'r') as f:
                        return f.read(), content_id
            return None, None
        except Exception as e:
            log(f"Error processing video {video['url']}: {e}")
            return None, None
        
    # Run all video processing tasks concurrently
    start_time = time.perf_counter()
    tasks = [process_video(video) for video in top_videos]
    results = await asyncio.gather(*tasks)
    elapsed = time.perf_counter() - start_time
    print(f"⏱️ Time taken to process videos: {elapsed:.2f} seconds")
    # Collect transcripts and IDs
    for transcript, cid in results:
